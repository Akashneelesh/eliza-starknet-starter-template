...
export default {
    ...
    handler: async (
        ...
        try {
            ...
            // Get the correct decimal value for the token
            const decimals = await erc20Token.decimals();
        
            // Convert the user input amount to an integer token value (handling both string and number inputs)
            const amountInUnits = BigInt(
                Math.floor(parseFloat(content.amount.toString()) * Math.pow(10, Number(decimals)))
            );
        
            // Ensure recipient address is correctly resolved
            const recipient =
                content.recipient ??
                (await getAddressFromName(account, content.starkName));
        
            // Prepare transfer call with the converted amount
            const transferCall = erc20Token.transferCall(recipient, amountInUnits);
        
            elizaLogger.success(
                `Transferring ${content.amount} tokens (converted to ${amountInUnits}) of ${content.tokenAddress} to ${recipient}`
            );
